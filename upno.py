...
def update_cron_with_random_time():
    ...
    new_cron_command = f"{random_minute} {random_hour} * * * cd {script_dir} && python3 {os.path.join(script_dir, 'upno.py')}\n"
    ...
    with open(cron_file, "w") as file:
        for line in lines:
            # Remove existing entry for `upno.py` if it exists
            if "upno.py" not in line:
                file.write(line)
